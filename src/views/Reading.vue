  ...
  <div class="min-h-screen bg-amber-50 p-6 font-kaiti flex flex-col items-center">
    ...
    <div class="w-full max-w-2xl bg-white rounded-3xl p-8 shadow-xl border-4 border-amber-200 relative overflow-hidden">
      ...
      <div v-if="!story" class="flex flex-col items-center justify-center py-10">
        ...
        <!-- 未登录提示 -->
        <div v-if="!canGenerate" class="mb-4 bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-center text-sm text-yellow-700">
          ⚠️ AI 故事需要登录后才能使用。
      ...
      <div v-else class="animate-fade-in">
        ...
        <div class="text-2xl leading-loose text-gray-700 text-justify">
          ...
          >
            {{ char }}
            <span
              v-if="!isKnown(char) && !isPunctuation(char)"
              class="absolute -top-3 left-1/2 transform -translate-x-1/2 text-[8px] bg-red-100 text-red-500 px-1 rounded opacity-0 group-hover:opacity-100"
            >生字</span>
          </span>
        </div>
...
import { storyApi, authApi } from '../utils/api';
...
// ★ 用 computed 实时判断是否可以生成（响应式）
const canGenerate = computed(() => {
  return userStore.isOnline && userStore.isLoggedIn;
});
...
const getCharClass = (char) => {
  if (isPunctuation(char)) return 'text-gray-700 cursor-default';
  if (isKnown(char)) return 'text-gray-700';
